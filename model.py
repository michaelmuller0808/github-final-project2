...
def csv_model_train(df,tag,prefix,test=False):
    ...
    ## update log
    update_train_log(tag,(str(dates[0]),str(dates[-1])),{'rmse':eval_rmse},runtime,MODEL_VERSION, MODEL_VERSION_NOTE)
...
def model_load(prefix='test',data_dir=None,training=True):
    ...
    ts_data = fetch_ts(data_dir)
    all_data = {}
    ts_data = {country: df for country, df in ts_data.items() if not df.empty}
    for country, df in ts_data.items():
...
def train_single_country(df,country,prefix,test=False):
    if len(df) > 30:
        return csv_model_train(df,country,prefix,test)
    else:
        return {}

def single_country_predict(country,year,month,day,all_models=None,all_data=None,test=False):
