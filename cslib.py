...
def fetch_ts(data_dir, clean=False):
    ...
    files = os.listdir(ts_data_dir)
    csv_files = [file for file in files if file.endswith('.csv')]
    if csv_files:
        return load_ts_data(ts_data_dir);
...
def load_data_from_csv(directory, prefix, country):
    return pd.read_csv(directory + "/" + prefix + country + ".csv")

def load_ts_data(ts_data_dir):
    # Check if directory contains files
    print("... loading ts data from files:" + ts_data_dir)
    ...
    for cf in os.listdir(ts_data_dir):
        ...
        if cf.endswith('.csv'):
            # Derive the key by removing the '.csv' extension and slicing the first 3 characters
            key = re.sub(r"\.csv$", "", cf)[3:]
            # Construct the file path
            file_path = os.path.join(ts_data_dir, cf)
            try:
                # Read the CSV file into a pandas DataFrame
                ts_data[key] = pd.read_csv(file_path)
            except Exception as e:
